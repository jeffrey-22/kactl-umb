/**
 * Author: Unknown
 * Description: Decremental Convex Hull
 * Status: not tested
 * Time: O(\log N)
 */
 
// TODO
// Seems extremely complicated and takes a lot of space, might not be worth it
// https://judge.yosupo.jp/submissions/?problem=convex_layers&order=%2Btime&status=AC
...
struct DecrementalUpperHull {
 ...
 public:
	explicit DecrementalUpperHull(const vector<Point> &ps) : n(ps.size()), seg(2 * n), s(n), alive(n) {
		assert(is_sorted(ps.begin(), ps.end()));
		links.reserve(n);
		for (int k = 0; k < n; ++k) {
			links.emplace_back(ps[k], nullptr, nullptr, k);
		}
		for (int k = 0; k < n; k++) {
